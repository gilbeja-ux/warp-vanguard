...
(async () => {
  ...
  await p.evaluate(() => {
    ...
    window.frame = function (now) {
      real(now);
      n2++;
      if (n2 === 8 && !window.__shots.peak) { bx.clearRect(0,0,buf.width,buf.height); bx.drawImage(src,0,0); window.__shots.peak = buf.toDataURL(); }
      if (n2 === 78 && !window.__shots.trough) { bx.clearRect(0,0,buf.width,buf.height); bx.drawImage(src,0,0); window.__shots.trough = buf.toDataURL(); }
    };
  });
  for (let i = 0; i < 140; i++) {
