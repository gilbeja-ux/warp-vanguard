...
// ---------- the CDP client lives in scripts/lib/cdp.js (shared with smoke.js) ----------
const { sleep, launchChrome, waitForPort, openPage } = require('./lib/cdp.js');
...
async function attachMac() {
  console.log(`launching Chrome${HEADLESS ? ' (headless)' : ''} on port ${PORT}`);
  chromeProc = launchChrome(PORT, { headless: HEADLESS });
...
(async () => {
  ...
  const { cdp, ws } = await openPage(PORT);
