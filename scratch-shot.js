...
(async () => {
  ...
  await p.evaluate(() => {
    ...
    window.frame = function (now) {
      real(now);
      const v = skyBreath();
      if (v > 2.2 && !window.__shots.peak) { bx.clearRect(0,0,buf.width,buf.height); bx.drawImage(src,0,0); window.__shots.peak = buf.toDataURL(); }
      if (v < 0.5 && !window.__shots.trough) { bx.clearRect(0,0,buf.width,buf.height); bx.drawImage(src,0,0); window.__shots.trough = buf.toDataURL(); }
    };
  });
  for (let i = 0; i < 140; i++) {
